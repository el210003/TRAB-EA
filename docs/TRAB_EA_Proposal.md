...
- **v1.12 as-built (multi-timeframe trend filter):** optional higher-timeframe (default M15) fast/slow EMA confirmation gate in `TryEnter` — `UseHTFConfirm`, `HtfTimeframe`, `HtfFastPeriod`, `HtfSlowPeriod`. When enabled, an entry is only taken if the HTF EMA pair agrees with the trade direction. A/B (EURUSD M1 2023–2025, real ticks) found the M15 gate is the first change to genuinely improve the signal: win rate lifted to ~34–40% (above the 2R break-even) and drawdown cut to 5–8% across out-of-sample sub-periods, yet the system remained marginally net-negative due to spread/slippage cost on the small-SL pin entries. Off by default to preserve live behavior.
